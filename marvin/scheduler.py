...
CREATE INDEX IF NOT EXISTS k_expires    ON key_pairs(expires);
    ...
    def get_activity(self):
        ...
        c.execute("SELECT DISTINCT o.name, o.project FROM schedule s, experiments e, owners o WHERE s.expid=e.id AND e.ownerid=o.id AND s.stop > ? - 604800 GROUP BY e.ownerid", (now,))
        distinct=c.fetchall()
        tasks["distinct active users (7d)"]=[{"name":x[0], "project":x[1]} for x in distinct] if distinct is not None else 0;
